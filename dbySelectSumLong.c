/*----------------------------------------------------------------------------
	Program : dbySelectSumLong.c
	Author  : Silver Hammer Software LLC
	Author  : Tom Stevelt
	Date    : 2019 - 2024
	Synopsis: Given table and where clause which selects one long field,
                return sum of found values.
	Return  : 
...
long dbySelectSumLong ( MYSQL *MySql, char *Table, char *Field, char *WhereClause, char *LogFileName )
{
	...
	if ( Query != NULL && Query->NumRows == 1 )
	{
		...
		{
			longValue = nsAtol( Query->EachRow[0] );
		}
		dbyFreeQuery ( Query );
		Query = (DBY_QUERY *) 0;
	}
